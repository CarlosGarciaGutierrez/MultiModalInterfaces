...
void GetSVMRepNumbers(char* Line,Vocabulary* VB,char * SVMRep,int LastResult,int* N1,int* N2)
{
 ...
 *N2=0; // Inicialization
 while(*Line && Word[0])
 {
  ...
  {
   if(First) // If there is a operand to asign
   {
    *N1=LastResult; // asign the operand
    *N2=N;
    First=false;
   }
   else
...
int MakeOperation(int Ope,int N1,int N2)
{
 int Res;
 switch(Ope)
 {
...
void CA(Vocabulary * VB,char * SVMTrainFile)
{
 ...
 while(strcmp(Buff,"exit"))
 {
   ...
   Pretrata(Buff);
   GetSVMRepNumbers(Buff,VB,SVMRep,LastResult,&N1,&N2);
   int Ope=PredictOpe(SVMTrainFile,SVMRep);
   Ope=ConfirmOpe(SVMTrainFile,SVMRep,Ope);
   LastResult=MakeOperation(Ope,N1,N2); 
