...
char * DuplicateStr(char * yytext,bool CaseSensitive);
%}
CHAR [A-Za-z]
WORD {CHAR}+
%%
{WORD}		{int Tok=Token(yytext); if(Tok>0) return Tok;}
\"{WORD}\"	{yylval= new Word(DuplicateStr(yytext,false),false); return LIMIT;}
\'{WORD}\'	{yylval= new Word(DuplicateStr(yytext,true),true); return LIMIT;}
.|\n		; 
%%
int yywrap()
{
...
static void ToBuffer(char * yytext,bool CaseSensitive=false)
{
 // Check the length 
...
char * DuplicateStr(char * yytext,bool CaseSensitive)
{
 ToBuffer(yytext+1,CaseSensitive); // Remove the first '"
